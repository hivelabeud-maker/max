...
function boot(w, h, search) {
  ...
  const api = new Function('window','document','performance','requestAnimationFrame','setTimeout','location','fetch','XMLHttpRequest','Image',
    ...
    ' TIMELINE:TIMELINE, RENDER:RENDER, Orbs:Orbs, Combo:Combo, Stick:Stick, restart:restart, handleTap:handleTap,' +
...
for (const [name,w,h] of DEVICES) {
  ...
  /* 상단침범 — 플레이 영역이 앱바 안으로 파고들면 실패.
     1차는 상단 카드가 침범하는 걸 막으려고 70 고정이었다. 이 프로젝트는
     앱바(HUD.APPBAR) 아래부터가 플레이 영역이므로 기준을 앱바로 바꾼다.
     검사의 의도(게임이 상단 UI 뒤로 들어가면 안 된다)는 그대로다 */
  const APPBAR = (a.HUD && a.HUD.APPBAR) || 70;
  if (S.pf.y < APPBAR) bad.push('상단침범');
...
line();
{
  ...
  let err = null, endAt = -1, peak = 0;
  try {
    a.run(60*28, (i, api)=>{

      if (api.Game.orbs && api.Game.orbs.count() > peak) peak = api.Game.orbs.count();
  ...
  const rows = [
    ...
    ['동시 최대 오브', peak + '개 (화면 밀도 지표)'],
    ['최종 점수', Math.round(G.score).toLocaleString('en-US') + '  (목표 ' + G.goal.toLocaleString('en-US') + ')'],
    ['터뜨린 오브', G.pops + '개'],
    ['최고 콤보', 'x' + (a.Combo ? a.Combo.best : 0)],
    ['잔여 오브', G.orbs.count() + '개 (라운드 클리어 후 0이어야 함)'],
    ['목표 달성', G.score >= G.goal ? '달성' : '실패 — 목표 미달 (goal 을 낮추거나 스폰을 올릴 것)'],
  ...
  /* ── 팝 수 하한 ─────────────────────────────────────────────────────────
     원본 영상은 콤보가 x76 까지 오르므로 최소 76개는 터진다. 21.5초에 100개
     남짓이 실측 체감이다. 1차 구현은 275개였는데, 그건 오브를 181개나 띄운
     결과였고 실장에게 "너무 과하다"고 지적받았다. 하한을 70으로 내린다. */
  if (G.pops < 70) { console.log('  ! 터뜨린 오브가 적습니다 (연출 밀도 부족)'); FAIL++; }
  if (G.orbs.count() > 0) { console.log('  ! 라운드 클리어 후 잔여 오브 존재'); FAIL++; }
  if (a.calls.cta !== 0) FAIL++;
  if (endAt < 0 || Math.abs(endAt - 21.50) > 0.25) { console.log('  ! 결과 화면 도달 시각이 타임라인과 어긋남'); FAIL++; }
  if (G.score < G.goal) { console.log('  ! 목표 점수 미달 — 클리어 연출이 거짓이 된다'); FAIL++; }
  /* ── 동시 오브 수: 하한과 ★상한을 함께 검사한다 ────────────────────────
     원본 영상 실측은 평온 구간 16~29개다. 1차 구현은 181개였고 그게
     "영상 샘플처럼 안 나온다"의 원인이었다 — 화면이 공으로 덮이면 이 게임의
     그림인 "검은 여백 위의 점 궤적과 폭발"이 사라진다.
     그래서 상한을 검사에 넣는다. 밀도를 올리고 싶어질 때 이 줄이 막는다. */
  if (peak < 12) { console.log('  ! 동시 오브 수가 적음 — 화면이 빈다'); FAIL++; }
  if (peak > 48) { console.log('  ! 동시 오브 수가 많음 — 검은 여백이 사라진다 (실측 16~29)'); FAIL++; }
  if (a.Combo && a.Combo.best < 30) { console.log('  ! 최고 콤보가 낮음 — HYPER FRENZY 단계에 도달 못 함'); FAIL++; }
}
...
line();
{
  ...
  console.log(pad('터뜨린 오브',18) + G.pops + '개');
  console.log(pad('최종 점수',18) + Math.round(G.score).toLocaleString('en-US'));
  if (err) FAIL++;
  if (G.state !== 'result') FAIL++;
...
/* ⑥ 전폭 딤(검정 띠) 금지 — 3회 재발한 실수라 빌드에서 막는다.
   ...
   ★ 예외 하나 — 상단 앱바 (ORB POP)
     이 프로젝트의 레퍼런스에는 화면 폭을 채우는 앱바가 있다. 규칙이 막으려던
     "플레이 화면 위에 깔리는 스크림"이 아니라 앱 UI 그 자체라, 규칙을 우회하지
     않고 여기에 명시적으로 예외를 뒀다: y + h <= APPBAR_MAX 안에서 끝나는
     채우기만 통과시킨다. 그 아래는 여전히 실패시킨다.
     우회해서 통과시키면 규칙 자체가 무력해진다 — 그래서 검사에 적었다. */
const APPBAR_MAX = 56;
console.log('\n⑥ 전폭 딤(검정 띠) 금지 검사');
line();
{
  ...
  let m;
  while ((m = re.exec(src))) {
    const y0 = parseFloat(m[2]), hh = parseFloat(m[3]);
    if (Math.abs(parseFloat(m[1])) <= 2 && Math.abs(hh) >= 6) {
      /* 앱바 영역 안에서 끝나면 통과 (위 주석의 명시적 예외) */
      if (y0 >= 0 && y0 + hh <= APPBAR_MAX) continue;
      hits.push(m[0]);
    }
  }
  if (hits.length) {
    ...
    console.log(pad('전폭 딤',22) + '0건 (전체화면 채우기 · 상단 앱바는 예외)');
  }
}
