...
- 착수 전 자문 7문과 프롬프트 3종 포함.
- **읽고 넘어가는 문서가 아니다.** `marketer`는 P0 게이트에서 렌즈 4개를 산출물로 내고 CH3·CH4·종합에서 각각 확정한다.
  `rfp-analyst`는 요구↔진짜문제 갈림 지점을, `critic`은 자문 5·7번을, `review-panel`은 관점별 렌즈를 강제로 낸다.
  대시보드로 조판되면 `mx-research-dashboard-design`이 렌즈별 착지 뷰를 지정한다.
...
- `proposal-psychology` — **제안·입찰 착수 전 심리 게이트**. 통념 4가지(손실 회피 / 고객사도 자기 문제를 모른다 / 담당자는 내부 보고를 통과해야 한다 / 자랑보다 의도와 과정)와 착수 전 자문 7문·프롬프트 3종. `marketer` P0가 자동 호출하고 `/proposal-psychology`로 직접도 부른다. 트리거 "제안 시작"/"입찰 준비"/"경쟁 PT"/"왜 우리인가"
- `campaign-copy-system` — **내러티브→슬로건→카피→UA 소재 4단계**. Step1 기승전결 타임라인 → Step2 슬로건 3안+키워드 의도 → Step3 목적별 카피 4유형(스케일·혜택 / 유머·밈 / 사자성어·언어유희 / 감성·IP 메타포) 각 3개 이상 → Step4 UA 소재 연계. **게임 커뮤니티 톤 게이트 포함**(밈 쓰기 전 최근 2주 게시물 30건 확인 강제). 트리거 "슬로건 뽑아줘"/"카피 리스트업"/"주년 카피"/"UA 소재 카피"
...
| **슬로건·카피**를 뽑아야 할 때 | `campaign-copy-system` |
| **제안·입찰 착수 직전** | `proposal-psychology` |
