...
- `mece-issue-tree` — 과업을 겹침 없이·빠짐 없이 쪼개 이슈트리·가설트리로. 트리거 "MECE로 쪼개줘", "문제 구조화", "뭐부터 봐야 하지"
- `source-grading` — 출처 신뢰도(A~F) × 정보 신빙성(1~6) 이원 등급 + 독립성 4시험 교차 확인. 마케터 T1~T4의 판단 근거를 남긴다. 트리거 "출처 등급", "교차 확인해줘", "이 숫자 믿을 만해"
- `insight-derivation` — 사실 → 관찰 → 긴장 → 인사이트 → 함의 5단 사다리. UX 5질문(여정 단계·막힌 일·없던 정보·대체한 것·뒤집을 지점)을 강제 통과시켜 제안의 명분을 만든다. 트리거 "인사이트 뽑아줘", "so what", "UX 관점에서"
