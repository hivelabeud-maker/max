...
> 상세 규칙은 `.claude/skills/anti-ai-writing/SKILL.md`, `.claude/skills/dumbify/SKILL.md`,
> `.claude/skills/easy-learning-writing/SKILL.md` 참조.
...
**easy-learning-writing — 다른 에이전트·스킬 산출물을 교육용으로 재구조화할 때 (구조 단위, dumbify 위에 얹는 레이어)**
- 내부 교육·가이드·온보딩·프로세스 설명·사용법·체크리스트·사내 시스템 UI·MAX OS 콘텐츠·AI 에이전트 사용법에 적용. 전략 보고서·리서치 원본·근거 자료에는 적용 안 함
- 문장 규칙은 dumbify를 그대로 상속. 여기서는 시작점(사용자 입장 먼저)·정보 순서(결론 먼저)·명사형 금지·이유 붙이기·Learning Structure 6단계(선택적)만 추가
- **원본 보호가 핵심**: 숫자·조건·정책·전략적 의미를 임의로 바꾸지 않는다. 원본과 충돌하면 원본 우선, 충돌 지점은 `⚠ 원본 확인 필요`로 표시

**적용 강도 구분**
| 글 종류 | anti-ai-writing | dumbify | easy-learning-writing |
|---|---|---|---|
| 채팅 답변·문서·메일 | 전체 적용 | 설명형일 때만 | 교육·가이드·온보딩류일 때만 |
| 영상 대본·릴스 스크립트 | 말하기용 부분집합만 | 가볍게 | 적용 안 함(storytelling·viral-hooks 우선) |
| 클라이언트 발송 문서 | 전체 + `tone-guardian` 경유 | 전체 | 해당 없음(외부 발송은 원문 유지가 우선) |
| 사내 교육·온보딩·에이전트 사용법 | 전체 적용 | 전체 적용 | 전체 적용(구조까지 재설계) |
