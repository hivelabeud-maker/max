...
- **모든 글·덱·카피 산출물의 최종 필터 = `dumbify` + `anti-ai-writing` 스킬을 Skill 툴로 직접 로드해 적용.**
  외부/내부 불문, 문장이 들어간 산출물은 발행 전 두 스킬을 불러와 통과시킨다.
  - `dumbify` — 일반인 5초 이해(전문용어 제거·한 문장 한 생각·구체 표현).
  - `anti-ai-writing` — **특히 카피(광고 문구·캡션·헤드라인·본문)는 항상 이 스킬로 AI 티/AI처럼 들리는 문장을 걷어낸다.**
  - 트리거 단어가 없어도 자동 적용. 이미 대화체인 짧은 캡션엔 가볍게.
- **출처는 항상 클릭 가능한 바로가기 링크.** 덱·리포트·회신 등 출처를 다는 모든 산출물에서
  매체명·레퍼런스를 텍스트로만 적지 말고 실제 원문 URL을 `<a href>`(HTML)/마크다운 링크로 연결한다.
  1차 URL이 없으면 그 수치를 읽은 2차 기사 URL을 건다.
- **HTML 산출물은 다운로드가 아니라 "바로 보기".** 덱·리포트 등 HTML은 Artifact로 퍼블리시해
  브라우저에서 바로 열리는 URL로 전달한다(파일 첨부만 하지 않는다). Artifact는 외부 폰트 CDN을 막으므로
  퍼블리시 사본은 CDN `<link>` 제거 + 시스템 폰트 폴백 + 래퍼 태그 제거(`<style>`+본문만). 상세는 mx-deck-design 스킬 "전달" 절 참조.
- **산출물 완료 후에는 마음대로 다음 단계로 넘어가지 않는다 — 항상 되묻는다.**
  리서치·발산·시안·덱 등 파이프라인 성격의 산출물을 하나 끝내면, 다음에 이어 붙일 수 있는
  **구체적 옵션**(어떤 에이전트/스킬로 무엇을 더 할 수 있는지)을 2~4개로 좁혀 제시하고 실장님 선택을 기다린다.
  - 예: 시장조사 완료 → "① concept-director로 관점 잡기 ② brainstormer로 발산 ③ design-trend-radar·reference-curator로 트렌드/레퍼런스 보강 ④ mx-deck-design으로 바로 덱화 — 어느 걸로 이어갈까요?"
  - 막연한 "다음 뭐 할까요?"가 아니라, **에이전트·스킬 이름과 그게 만들어낼 것을 구체적으로** 걸어 고르기 쉽게 한다.
  - 다음 단계가 뻔한 단순 잡업(오타 수정 등)에는 이 규칙을 적용하지 않는다.
