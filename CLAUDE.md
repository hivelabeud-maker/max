...
└── library/
    ...
    ├── proposal-corpus.md     # ★ 실제 수주 제안서·발표 대본에서 뽑은 골격 (CH5~CH7 기준)
...
**⑥ 역할·파이프라인 오케스트레이터**
- `marketer` (마케터) — RFP부터 발표 원고까지 **7챕터 자동 체이닝**. `0 RFP 분석 → CH1 현황 조사(팩트) → CH2 진단(포지셔닝·빈자리) → CH3 문제 정의(한 문장) → CH4 발산(방향 후보) → CH5 전략 확정 → CH6 제안서 구성 → CH7 발표 원고`. 담당: rfp-analyst · market-research · design-trend-radar · reference-curator · brainstormer (CH2·CH3·CH5·CH6은 marketer 자체 수행, CH7은 `proposal-script` 스킬). **라우팅 구분**: "마케터 돌려"는 CH4까지, **"제안서까지 돌려줘"/"마케터 풀로"는 CH7까지** 사람 개입 없이 간다. 산출 포맷은 `mx-research-dashboard-design`(리서치)와 `mx-deck-design`(제안 덱).
  - **v9 게이트 5개** — G1 조사 충분성(CH1→CH2) · G2 축의 근거(CH2→CH3) · G3 실행 가능성(CH4→CH5, critic 60점 미만 불통과) · G4 다각도 검토(CH6→CH7, review-panel 🔴 0건) · G5 사실·톤(CH7→종료, fact-checker + tone-guardian). 걸리면 지정 챕터로 되돌아가고 그 사실을 먼저 알린다.
  - **CH5~CH7 기준 문서는 `.claude/library/proposal-corpus.md`** — 실제 수주한 코웨이 BEREX 제안서(68슬라이드 7장)와 발표 대본(21분 21초)을 해체해 만든 골격. 제안서 골격 A형(컨셉 제안 7장)/B형(수행 제안 10장) 판정, 선언 슬라이드 배치, 슬라이드당 18.8초 분량 기준, 강점 모듈 14종. **일반적인 제안서 구조를 새로 지어내지 않는다.**
  - **MECE 적용** — CH2 분류·포지셔닝 축, CH3 문제 분해, CH5 전략 축에서 겹침·빠짐을 자문한다.
  - **v8.1 품질 규칙**: CH1은 조사 6스텝(S1 카테고리 정의·S2 시장규모·S3 공급측·S4 수요측·S5 이종업계·S6 트렌드) 고정 — "빠르게"여도 S1·S2·S3·S6은 필수, "제대로"면 S4·S5까지. 출처는 T1~T4 티어를 달고 T4는 본문 금지, 핵심 수치는 교차 확인 2건, 못 찾으면 DATA GAP. 글은 초등 5학년 기준 `용어(쉬운 풀이)` 형식.
...
- `proposal-script` — **제안서 → 발표 원고(대본) 변환**. 슬라이드별 대사 + 초 수 + 장 전환 브릿지 문장. 코웨이 BEREX 발표 대본(21분 21초, 68슬라이드) 실측 규격 — 오프닝은 개인 경험 훅 30~50초, 슬라이드당 평균 18.8초, 숫자 강조어는 3~4개만, 클로징 두 문장. 마케터 CH7이 호출. 트리거 "발표 원고"/"대본 써줘"/"PT 대본"/"발표 연습용"
