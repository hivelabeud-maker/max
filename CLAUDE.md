...
- 응답은 간결하고 실무 중심
- 한국어 소통 (별도 요청 시 영어)
- **새 세션·새 대화에서도 히스토리를 다시 입력받지 않는다** — 프로젝트/고객사가 언급되면 먼저 `.claude/projects/`에서 같은 프로젝트명·고객사 폴더나 노트(`YYYYMM_프로젝트명.md` 또는 `YYYYMM_PROJECT/`)가 있는지 찾아 **요약 섹션부터 읽고 시작**한다. 있으면 그 결정사항·진행 상태를 이어받아 작업하고, 사용자에게 처음부터 다시 설명해달라고 요청하지 않는다. 없으면 새로 시작하되 노트를 생성한다(`project-note-rules.md` 구조 준수).
...
## 현재 등록된 에이전트 (17종 · 4그룹 + 역할·파이프라인 오케스트레이터)
**① 수주·전략**
