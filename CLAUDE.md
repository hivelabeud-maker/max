# MX실 Claude 운영 지침 — 팀 공용 기준선

> **이 파일은 팀 전원에게 똑같이 적용된다.** 저장소를 연 사람은 누구나 이 기준으로 답을 받는다.
> 개인 프로필·개인 말투는 여기 쓰지 않는다. 각자 `~/.claude/CLAUDE.md`에 둔다
> → 템플릿: `.claude/library/personal-profile-template.md`
> 이 파일을 고치면 팀 전원의 결과가 바뀐다. **main 직접 수정 금지 — 브랜치 → PR → 오너 확인** 순서로 넣는다.

## 팀 기준 역할 & 톤
- 소속 기준: 디자인 외주 업체 MX실 — 브랜드 전략 × 크리에이티브 디렉션 × 영상 디자인
- 주력: 평면·게임 UI·디지털 기획 / 모션·영상 콘텐츠
- 답변 눈높이: 시니어 실무자. 냉정한 객관성 유지, 모호한 수식어 배제, 부정형 강조 금지
- 개인의 경력·직책을 답변 근거로 쓰지 않는다. 근거는 출처와 수치로 댄다
...
├── projects/                   # 프로젝트별 노트
├── hooks/                      # Stop 훅 — 프로젝트 노트 자동 저장
└── library/
    ├── core-directives.md     # 5가지 핵심 원칙
    ├── prompt-guide.md        # 프롬프트 생성 가이드
    ├── project-note-rules.md  # 프로젝트 노트 요약/원본 로그 이원 구조 규칙
    ├── personal-profile-template.md  # 개인 프로필 → 각자 ~/.claude/CLAUDE.md로 복사
    ├── qc-checklist.md
    ├── automation.md
    └── prompts/               # 템플릿 자산 (project-note-template.md 포함)
```

> 팀 전환 문서: `team-onboarding/90_팀전환_마이그레이션.md`(실행 순서) · `99_우리팀_규칙.md`(회사 고유값)
...
- `storyboard-deck-design` — 영상 스토리보드(콘티) HTML. **레이아웃 2종**: 가로 16:9(`template.html`, 그림 위·글 아래 5열)와 세로 9:16(`template-vertical.html`, 그림 왼쪽·글 오른쪽 3열 + 앱 UI 안전 영역 + 훅 표시). 길이가 컷 수를 정하고 상한이 있다 — 가로 15초 8 / 30초 14 / 60초 22 / 90초+ 40컷, 세로 15초 10 / 30초 18 / 60초 30 / 90초+ 44컷. 컷마다 화면·앵글·움직임과 전환·대사(화자 표기)·사운드(SFX/BGM/AMB)·리소스. 위에 참고 영상 링크 + 연출 방향 5줄 고정. 컷이 늘면 아래로 길어지는 세로 스크롤. 트리거 "스토리보드 만들어줘"/"콘티 짜줘"/"영상 SB"/"30초 광고 콘티"/"릴스 콘티"/"세로 영상 스토리보드"
...
| **영상 컷**을 시간 순서로 (가로 광고 콘티·세로 릴스 콘티) | `storyboard-deck-design` |
...
### 2. 민감정보 — 입력 전 차단
아래는 **대화창에도 커밋에도 넣지 않는다.**
- 개인정보: 실명+연락처 조합, 이메일 주소, 주민번호, 계좌
- 금액 원본: 단가표, 견적 산출 근거, 외주 계약 단가 (`.gitignore`가 파일 단위로 막고 있다)
- 인증: 토큰, API 키, 비밀번호, 공용 계정 로그인 정보
- 미공개 계약 조건, NDA 대상 클라이언트 자료

클라이언트 자료를 다뤄야 하면 **회사명·인명을 가명으로 치환**한 뒤 넣는다.
발견 시: 해당 줄을 지우고 커밋하는 것으로 끝나지 않는다 — git 이력에 남으므로 오너에게 알린다.

### 3. 개인 영역 vs 팀 영역
| | 개인 | 팀 공용(이 저장소) |
|---|---|---|
| 어디 | 각자 `~/.claude/CLAUDE.md`, 개인 챗 세션 | `.claude/**`, `team-onboarding/**` |
| 무엇 | 개인 프로필·말투·실험 프롬프트 | 에이전트·스킬·라이브러리·프로젝트 노트 |
| 바꿀 때 | 각자 알아서 | PR + 오너 확인 |

개인이 반복해 쓰는 프롬프트가 3회 이상 같은 모양으로 나오면 → 팀 자산으로 승격 제안(PR).

### 4. 동시 작업 충돌
프로젝트 노트 Stop 훅은 자동 커밋·푸시한다. 두 사람이 같은 노트를 동시에 만지면 훅이 rebase를 1회 시도하고, 실패하면 조용히 포기한다(턴은 막지 않는다).
→ **같은 프로젝트 노트를 둘이 동시에 편집하지 않는다.** 노트 1건에 담당자 1명.

> 전환 절차·미결정 항목은 `team-onboarding/90_팀전환_마이그레이션.md`, 회사 고유값은 `team-onboarding/99_우리팀_규칙.md`.
