...
line();
{
  ...
  /* ── 동시 적 수: 하한과 ★상한을 함께 검사한다 ──────────────────────────
     실게임 참고 영상 실측: 평온 구간 28~35개, 전투 중(폭발 이펙트 포함) 110~157개.
     1차 구현은 680마리였고 실장에게 "몹이 몰려오는 게 그대로"라고 지적받았다 —
     그 수는 "몰려온다"가 아니라 "화면이 덮인다"로 보인다.
     하한만 두면 밀도를 올리고 싶은 충동이 그대로 재발하므로 상한을 넣는다. */
  if (peak < 90)  { console.log('  ! 동시 적 수가 적음 — 화면이 빈다'); FAIL++; }
  if (peak > 260) { console.log('  ! 동시 적 수가 많음 — 화면이 덮인다 (실게임 실측 28~157)'); FAIL++; }
}
