  ...
  update: function (dt) {
    ...
    if (Game.state === 'battle' || Game.state === 'tutorial') {
      ...
      for (var i = 0; i < ws.length; i++) {
        ...
        while (this.acc[i] >= 1) {
          this.acc[i] -= 1;
          if (Game.enemies.count() < 680) Enemies.spawn(w.t, w.spdM || 1, w.hpM || 1);
        }
      }
    }
    ...
    if (this.ultT >= 0) {
      this.ultT += dt;
      if (this.ultT >= 0.55) { this.ultT = -1; Game.timeScale = 1; Skills.ultimate(); }
    }
    if (Game.state === 'ultimate' && this.ultT < 0) {
      Game.enemies.each(function (e) { if (e.dieT <= 0) Enemies.kill(e); });
    }
